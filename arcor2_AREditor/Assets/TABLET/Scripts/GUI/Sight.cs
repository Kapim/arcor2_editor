        ...
        private void Update() {
            if (SelectorMenu.Instance.Active) {
                ...
                if (SelectedGizmoAxis?.GetInvocationList().Length > 0) {
                    ...
                    foreach (RaycastHit gizmoHit in hits) {
                        if (gizmoHit.collider.gameObject.CompareTag("gizmo_x")) {
                            SelectedGizmoAxis.Invoke(this, new GizmoAxisEventArgs(Gizmo.Axis.X));
                        } else if (gizmoHit.collider.gameObject.CompareTag("gizmo_y")) {
                            SelectedGizmoAxis.Invoke(this, new GizmoAxisEventArgs(Gizmo.Axis.Y));
                        } else if (gizmoHit.collider.gameObject.CompareTag("gizmo_z")) {
                ...
                if (Physics.Raycast(ray, out RaycastHit hit)) {
                    ButtonInteractiveObject button = hit.collider.GetComponent<ButtonInteractiveObject>();
                    if (button != null) {
                        if (buttonInteractiveObject != null) {
                            if (buttonInteractiveObject != button) {
